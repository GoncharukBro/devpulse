...
collectedData: {
  employees: Array<{
    ...
    topTasks: Array<{ id: string; summary: string; type: string; spentMinutes: number }>
  }>
} | null
```
...
3. **ТОП-20 задач** по spent time (~1000-1600 символов)
   - ID, summary, тип, потраченное время, статус (завершена/просрочена)
...
### Агрегация данных employee из нескольких подписок

Если сотрудник участвует в нескольких подписках (проектах), для типа `employee` данные собираются по каждой подписке отдельно, затем объединяются:
- Числовые метрики (задачи, время) — суммируются
- Процентные KPI (utilization, completion rate) — усредняются
- topTasks — объединяются и пересортировываются по spentMinutes, берутся топ-20
- issuesByType — объединяются (суммы по категориям)
