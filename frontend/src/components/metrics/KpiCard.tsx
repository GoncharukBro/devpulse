    ...
    return (
      <Card>
        <div className="animate-pulse">
          <div className="mb-3 h-4 w-24 rounded bg-gray-700/50" />
  ...
  return (
    <Card>
      ...
      <div className="mt-2 flex items-end justify-between">
        <span className={`text-2xl font-bold ${value !== null ? colors.text : 'text-gray-500'}`}>
          {value !== null ? <AnimatedNumber value={value} suffix={suffix} /> : 'Н/Д'}
        </span>
        <TrendIndicator trend={trend ?? null} />
      </div>
      {trendValue && (
        <div className="mt-1 text-xs text-gray-500">{trendValue}</div>
      )}
    </Card>
  );
}
