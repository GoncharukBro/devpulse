...
export default function MetricTooltip({ metric, children }: MetricTooltipProps) {
  ...
  return (
    ...
    >
      {children ?? (
        <button type="button" className="cursor-help text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 rounded" aria-label={data.title}>
          <Info size={14} />
        </button>
      )}
      {visible && (
