...
export default function LlmQueueIndicator({ items }: LlmQueueIndicatorProps) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-gray-200 dark:border-surface-border bg-gray-50 dark:bg-surface-light px-4 py-2.5 text-sm">
      <Bot size={16} className="text-purple-400" />
      {items.length === 0 ? (
        <span className="text-gray-400 dark:text-gray-500">LLM-анализ: очередь пуста</span>
      ) : (
