...
export default function TeamMembersList({ members, loading }: TeamMembersListProps) {
  ...
  if (!members.length) {
    return (
      <Card>
        <p className="text-center text-sm text-gray-500">Нет участников</p>
      </Card>
    );
  }
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-surface-border">
              <th className="px-3 py-3 text-left text-xs font-medium uppercase text-gray-500">Имя</th>
              ...
              <th className="px-3 py-3 text-left text-xs font-medium uppercase text-gray-500">Проекты</th>
              <th className="px-3 py-3 text-left text-xs font-medium uppercase text-gray-500">Тренд</th>
            </tr>
          </thead>
          <tbody>
            {members.map((m) => {
