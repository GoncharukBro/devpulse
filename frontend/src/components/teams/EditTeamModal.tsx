        ...
        <div>
          ...
          <div className="space-y-1">
            {activeMembers.map((m) => (
              <div
                key={m.youtrackLogin}
                className="flex items-center justify-between rounded-lg px-3 py-2 text-sm hover:bg-surface-lighter"
              >
                <div>
                  <span className="text-gray-200">{m.displayName}</span>
                  <span className="ml-1 text-xs text-gray-500">({m.youtrackLogin})</span>
                </div>
                <button
