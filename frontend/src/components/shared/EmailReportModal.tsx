...
export default function EmailReportModal({
  ...
  const fetchData = useCallback(async (): Promise<EmailCache> => {
    ...
    const data = await reportsApi.emailPreview({ type, youtrackLogin, subscriptionId, teamId });
    setCache(data);
    return data;
  }, [cache, type, youtrackLogin, subscriptionId, teamId]);

  // Load subject on open
  useEffect(() => {
    if (!open) {
      setCache(null);
      return;
    ...
    reportsApi
      .emailPreview({ type, youtrackLogin, subscriptionId, teamId })
      .then((data) => {
        if (!cancelled) {
          setCache(data);
        }
      ...
      .catch(() => {
        if (!cancelled) {
          toast.error('Не удалось загрузить данные для письма');
        }
      })
      .finally(() => {
  ...
  }, [open, type, youtrackLogin, subscriptionId, teamId]);
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            disabled
            className="w-full rounded-lg border border-gray-200 dark:border-surface-border bg-gray-100 dark:bg-surface-lighter px-3 py-2 text-sm text-gray-600 dark:text-gray-300 placeholder-gray-400 dark:placeholder-gray-600 opacity-50"
          />
        </div>
