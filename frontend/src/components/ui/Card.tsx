...
export default function Card({ children, className = '', noPadding, header, footer }: CardProps) {
  return (
    <div
      className={`flex flex-col rounded-xl border border-gray-200 dark:border-surface-border bg-white shadow-sm dark:shadow-none dark:bg-surface transition-colors ${className}`}
    >
      {header && (
        <div className="border-b border-gray-200 dark:border-surface-border px-6 py-4">{header}</div>
      )}
      <div className={`flex flex-1 flex-col ${noPadding ? '' : 'p-4 sm:p-6'}`}>{children}</div>
      {footer && (
